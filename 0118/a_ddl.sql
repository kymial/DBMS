...
# 필드 / 컬럼
# 필드 : 데이터의 특정 유형 지정, 각 필드는 고유한 이름과 데이터 타입을 가지고 있다.
...
#	 : 선택하지 않을 경우 해당 데이터베이스 내의 구성요소를 사용할 수 없다.
...
# 4. 데이터 베이스 삭제 (drop database)
# : 데이터베이스를 삭제하는 기능, 해당 작업은 실행 후 되돌릴 수 없다.
# 기본형태
# drop database databhse_name;
...
# : 테이블의 구조, 테이블에 정의된 컬럼, 데이터 타입, 키 정보(제약조건) 등을 조회하는데 사용

use example;
-- 왼쪽 Navigator의 Schemas 필드의 데이터베이스명을 더블 클릭해도 같은 효과
# 기본 형태
# describe table_name;
...
# Key(각 컬럼의 제약상항-키), Default(기본값 지정), Extra(제약상항-추가옵션)
...
# a. 컬럼 추가(add column)
# 기본형태 : alter table table_name add column_name column_definitior;
#		  add column_name column_definitior;
...
# : if exists 사용 시 존재할 때만 삭제하기 때문에 오류가 발생하지 않는다.

drop table if exists `lectures`; -- 오류 발생 시 해당 위치부터 그 아래의 실행은 안된다.
