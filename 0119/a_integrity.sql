...
# 도메인 : 데이터 타입, 길이, 가능한 값의 범위
# 예) 은행 계좌에 입금되는 금액 - 양의 정수만을 허용하는 unsigend 설정 (unsigend => 양수만 취급하는)

# 4. 사용자 정의 무결성
# : 특정 어플리케이션의 규칙에 따라 정의
# : 구현하고자 하는 비즈니스 규칙이나 애플리케이션의 특정 요구 상항을 반영
# 예) A 홈페이지에서는 회원가입 시 비밀번호 설정을 6 ~ 8 자 이내, 영문자, 숫자, 특수문자 포함해야함 
