...
-- select 컬럼문(열 목록) : 원하는 열(컬럼)을 지정
-- from 테이블명 :어떤 테이블에서 데이터를 가져올지 지정
-- where 조건 : 특정 조건에 맞는 데이터만 선택
-- group by 그룹화 할 열 이름 : 특정 열을 기준으로 그룹화
-- having 그룹 조건 : 그룹화 한 이후의 조건을 지정
-- order by 정령 열이름 : 결과를 특정 열의 순서로 정렬
...
# : 해당 테이블의 전체 컬럼을 조회할 때 * 기호를 사용하여 전체 컬럼을 지정

select * from `korea_db`.`member`;

# 2개 이상의 여러 컬럼 조회를 위해서는 해당 컬럼명을 , 로 구분하여 나열
...
select `member_id` "맴버 아이디", `name` as "맴버"
	from `korea_db`.`member`;
...
select * 
	from `korea_db`.`member`
		where area_code = 'BUSAN' and grade = 'Silver';

# or : 두 조건 중 하나라도 참(조건 중에 하나라도 참이면 참)
# 지역코드(area_code)가 SEOUL 이거나 BUSAN인 회원을 조회
select * 
	from `korea_db`.`member`
		where area_code = 'SEOUL' or area_code = 'BUSAN';
...
# : null이 '값이 없음'을 나타내며, 그 어떤 값과도 비교할 수 없음
# : null 여부를 확인하는 키워드
# 	is null, is not null
# is null
...
# points가 200 이상이면서 400이하인 데이터 추출
...
# 위 아래 둘다 결과 똑같음        
# between A and B A에서 B까지 (숫자와 날짜만)

select *
	from `korea_db`.`member`
		where points between 200 and 400;

#! in(지정할 범위의 문자 데이터를 나열)
# : 지정된 리스트 중 하나와 일치 하면 잠
# : 문자로 표기된 데이터에 범위를 추가할 때

select member_id, name, area_code
	from `korea_db`.`member`
...
select *
	from `member`
		where join_date between "2023-01-01" and "2024-01-23";
